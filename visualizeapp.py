...
try:
    digit_net.load_state_dict(
        torch.load("weights/digit_net_best.pth", map_location=device, weights_only=True)
    )
    operator_net.load_state_dict(
        torch.load(
            "weights/operator_net_best.pth", map_location=device, weights_only=True
        )
    )
